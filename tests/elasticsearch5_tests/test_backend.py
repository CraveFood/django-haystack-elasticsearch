...
from tests.core.models import AFourthMockModel, AnotherMockModel, ASixthMockModel, MockModel
...
class TestSettings(TestCase):
    def test_kwargs_are_passed_on(self):
        from haystack_elasticsearch.elasticsearch import ElasticsearchSearchBackend
...
class Elasticsearch5SearchBackendTestCase(TestCase):
    ...
    def test_non_silent(self):
        bad_sb = connections['default'].backend('bad', URL='http://omg.wtf.bbq:1000/', INDEX_NAME='whatver',
    ...
    def test_update_no_documents(self):
        ...
        sb = connections['default'].backend('default', URL=url, INDEX_NAME=index_name,
                                                  SILENTLY_FAIL=False)
        try:
            sb.update(self.smmi, [])
            self.fail()
...
class Elasticsearch5FacetingTestCase(TestCase):
    ...
    def test_facet(self):
        ...
        counts = SearchQuerySet('default').filter(content='white').facet('facet_field',
    ...
    def test_multiple_narrow(self):
        self.sb.update(self.smmi, self.sample_objs)
        counts = SearchQuerySet('default').narrow('editor_exact:"Perry White"').narrow(
    ...
    def test_narrow(self):
        self.sb.update(self.smmi, self.sample_objs)
        counts = SearchQuerySet('default').facet('author').facet('editor').narrow(
    ...
    def test_date_facet(self):
        ...
        # Facet by day
        counts = SearchQuerySet('default').date_facet('pub_date', start_date=start, end_date=end,
